  ...
  return (
    <div className="space-y-3">
      <div className="rounded border border-[var(--border)] bg-[var(--panel)] p-3">
        <h4 className={`${subsectionTitleClassName} text-white`}>Project lighting summary</h4>
        ...
        <ul className="space-y-2">
          {roomsWithLights.map((room) => {
            ...
            return (
              <li
                key={room.roomId}
                className={`rounded border border-[var(--border)] border-l-4 bg-[var(--panel)] p-3 ${bandClass(
              ...
              >
                ...
                <dl className="mt-2 grid grid-cols-2 gap-x-2 gap-y-1 text-xs">
                  ...
                  <dd className="text-right text-white">{room.targetLux} lux</dd>
                  <dt className="text-[var(--muted)]">Geplaatst (in ruimte)</dt>
                  <dd className="text-right text-white">{room.placedQuantityInsideRoom}</dd>
                  <dt className="text-[var(--muted)]">Product(en)</dt>
                  <dd className="text-right text-[10px] text-zinc-300">{productLabels || "—"}</dd>
                  ...
                  {showComplianceStatus ? (
                    <>
                      ...
                      <dd
                        className={`text-right font-medium ${
                          compliance.meetsTarget
                            ? "text-[var(--success)]"
                            : "text-[var(--error)]"
                        }`}
                      >
                        {compliance.meetsTarget ? "🟢 Meets target" : "🔴 Below target"}
                      </dd>
                    </>
                  ) : null}
                </dl>
